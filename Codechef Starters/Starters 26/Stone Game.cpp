...
// Problem: Stone Game
// Contest: CodeChef - CodeChef Starters 26 Division 2 (Unrated)
// URL: https://www.codechef.com/START26B/problems/STNGAME
// Memory Limit: 256 MB
// Time Limit: 1000 ms
// Problem tag: gready, 2-pointers
// TC:
// SC:
...
void solve()
{
	...
	sort(all(a));
	sort(all(b),cmp);
	ll lo1=0,lo2=0,hi1=n-1,hi2=n-1;
	ll i=0,j=2*n-1;
	string ans;
	for (ll i=0;i<2*n;i++) ans.pb('X');
	ll turn=1;
	while (i<=j && (lo1<=hi1) && (lo2<=hi2))
	{
		if (turn)	// Alice, lexi: smaller
		{
			if (a[lo1]<b[lo2])
			{
				ans[i]=a[lo1];
	...
	while (lo1<=hi1) ans[i++]=a[lo1++];
	while (lo2<=hi2) ans[i++]=b[lo2++];
	cout<<ans;
}
